...
void test_triangle::test_fronts_validation() {
    ...
    QVERIFY(not Triangle({{{0, 10.1}, {1, 10}, {2, 10}}},
                         {{0, 60}, {1, 60}, {2, 60}}, 1)
                    .isValidTriangle());
    QVERIFY(not Triangle({{{0, 10.5}, {1, 10}, {2, 10}}},
                         {{0, 60}, {1, 60}, {2, 60}})
                    .isValidTriangle());
...
void test_triangle::test_3fronts_calculating() {
    ...
    QCOMPARE(Triangle({{0, 10}, {1, 10}, {2, 10}}, {{0, 90}})
                 .anglesAsMap()
                 .values(),
             QList<double>({90}));
}
...
void test_triangle::test_angles_filling() {
    ...
    // with 1 known angle
    QCOMPARE(
        Triangle({{0, 3}, {1, 4}, {2, 5}}, {{2, 90}}).anglesAsMap().values(),
        QList<double>({37, 53, 90}));
    // test with a wrong angle
    QCOMPARE(Triangle({{0, 37}, {1, 40}, {2, 54}}, {{0, 48}})
                 .anglesAsMap()
                 .values(),
             QList<double>({48}));
}
...
void test_triangle::test_2fronts_triangle() {
    ...
    // fronts: 93, 67, (89)
    // angles: 72, (43), 65
    QCOMPARE(Triangle({{0, 93}, {1, 67}}, {{0, 72}, {2, 65}})
                 .frontsAsMap()
                 .values(),
    ...
    // fronts: 19, (15), 13
    // angles: 85, 52, 43
    QCOMPARE(Triangle({{0, 19}, {2, 13}}, {{1, 52}, {2, 43}})
                 .frontsAsMap()
                 .values(),
